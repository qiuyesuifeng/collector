...
func (s *Section) ToString() string {
	...
	return fmt.Sprintf("Section(%+v)", *s)
}
...
func (c *Config) String() string {
	...
	for k, v := range c.data {
		sectionValue += fmt.Sprintf("Section([key]%s[value]%s )", k, v.ToString())
	}

	return fmt.Sprintf("Config([data]%s[file]%s[Comment]%s[Spliter]%s)", sectionValue, c.file, c.Commet, c.Spliter)
}
...
func (c *Config) Unmarshal(v interface{}) error {
	...
	for i := 0; i < n; i++ {
		...
		case reflect.Map:
			...
			vf.Set(m)
		default:
			return errors.New(fmt.Sprintf("cannot unmarshal unsuported kind: %s into struct field: %s", vf.Kind().String(), tf.Name))
		}
	}
	return nil
}
